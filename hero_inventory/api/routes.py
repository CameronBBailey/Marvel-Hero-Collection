...
def create_hero(current_user_token):
    ...
    print(f'Welcome your current token is: {current_user_token.token}')

    hero = Hero(name,description,comics,powers,date_created,user_token = user_token )
...
def get_drones(current_user_token):
    ...
    drones = Hero.query.filter_by(user_token = owner).all()
    response = heroes_schema.dump(drones)
    return jsonify(response)
